...
const DEFAULT_MAX_SOLDIERS: i32 = 8;
...
impl SaveManager {
    ...
    pub fn load_save(save_path: &Path, editor_exe: &Path) -> Result<Value> {
        ...
        if !editor_exe.exists() {
            return Err(anyhow!("编辑�?EXE 不存在：{}", editor_exe.display()));
        }

        let json_path = save_path.with_extension("json");
        info!("正在将二进制转换�?JSON：{:?}", json_path);
    ...
    pub fn export_json(json_value: &Value, export_path: &Path) -> Result<()> {
        ...
        info!("已导�?JSON 到：{}", export_path.display());
        Ok(())
    }
    ...
    pub fn sync_class_skill(
        ...
        if skill_upgrade_id.is_none() {
            info!(
                "�?英雄 {} 没有有效�?skillUpgrade 字段，跳过兵种技能同�?,
                hero_record_id
            );
            return Ok(());
        }
    ...
    pub fn modify_hero_soldiers_lost(
        json_value: &mut Value,
        hero_record_id: &str,
        soldiers_lost: i32,
    ) -> Result<()> {
        ...
        for member in members.iter_mut() {
            if member["name"].as_str() == Some("soldiersLost") {
                member["value"] = Value::Number(soldiers_lost.into());
                info!("Modified soldiersLost for hero {} to {}", hero_record_id, soldiers_lost);
                return Ok(());
            }
        }

        Err(anyhow!("soldiersLost field not found for hero {}", hero_record_id))
    }

    pub fn modify_hero_max_soldiers(
        json_value: &mut Value,
        hero_record_id: &str,
        new_max_soldiers: i32,
    ) -> Result<()> {
        let hero_record = json_value["records"][hero_record_id].as_object_mut()
            .ok_or_else(|| anyhow!("Hero record {} not found", hero_record_id))?;
        ...
        for member in members.iter_mut() {
            if member["name"].as_str() == Some("maxSoldiers") {
                member["value"] = Value::Number(new_max_soldiers.into());
                info!("Modified maxSoldiers for hero {} to {}", hero_record_id, new_max_soldiers);
                return Ok(());
            }
        }

        members.push(serde_json::json!({"name": "maxSoldiers", "type": "Int32", "value": new_max_soldiers}));
        info!("Inserted missing maxSoldiers field for hero {} with value {}", hero_record_id, new_max_soldiers);
        Ok(())
    }
    ...
    pub fn modify_hero_squad_members(
        json_value: &mut Value,
        hero_record_id: &str,
        new_max_soldiers: Option<i32>,
        new_current_count: Option<i32>,
    ) -> Result<String> {
        if new_max_soldiers.is_none() && new_current_count.is_none() {
            return Err(anyhow!("必须至少指定原始人数或当前人数之一"));
        }
        if let Some(max) = new_max_soldiers {
            if max <= 0 {
                return Err(anyhow!("原始人数必须大于 0，输入值为 {}", max));
            }
        }
        if let Some(current) = new_current_count {
            if current < 0 {
                return Err(anyhow!("当前人数不能为负数，输入值为 {}", current));
            }
        }

        let (current_max_soldiers, current_soldiers_lost) = {
            let record = json_value["records"][hero_record_id].as_object()
                .ok_or_else(|| anyhow!("Hero record {} not found", hero_record_id))?;
            let members = record["members"].as_array()
                .ok_or_else(|| anyhow!("Members array not found in hero record"))?;

            let mut max_s = DEFAULT_MAX_SOLDIERS;
            let mut lost = 0i32;
            for m in members {
                match m["name"].as_str() {
                    Some("maxSoldiers") => { if let Some(v) = m["value"].as_i64() { max_s = v as i32; } }
                    Some("soldiersLost") => { if let Some(v) = m["value"].as_i64() { lost = v as i32; } }
                    _ => {}
                }
            }
            (max_s, lost)
        ...
        let old_current = (current_max_soldiers - current_soldiers_lost).max(0);
        let final_max = new_max_soldiers.unwrap_or(current_max_soldiers);

        if let Some(new_current) = new_current_count {
            if new_current > final_max {
                return Err(anyhow!("当前人数 {} 不能超过原始人数 {}", new_current, final_max));
            }
        }

        let (final_soldiers_lost, auto_adjusted) = if let Some(new_current) = new_current_count {
            (final_max - new_current, false)
        } else {

            if old_current > final_max {
                (0, true)
            } else {
                (current_soldiers_lost, false)
            }
        };

        let mut log_parts = Vec::new();
        if let Some(new_max) = new_max_soldiers {
            log_parts.push(format!("原始人数 {} �?{}", current_max_soldiers, new_max));
        }
        if let Some(new_current) = new_current_count {
            log_parts.push(format!("当前人数 {} �?{}", old_current, new_current));
            log_parts.push(format!("已损�?{} �?{}", current_soldiers_lost, final_soldiers_lost));
        } else if auto_adjusted {
            let new_current_display = final_max - final_soldiers_lost;
            log_parts.push(format!(
                "当前人数超出新上限，自动调整 {} �?{}",
                old_current, new_current_display
            ));
            log_parts.push(format!("已损�?{} �?{}", current_soldiers_lost, final_soldiers_lost));
        }

        let record = json_value["records"][hero_record_id].as_object_mut()
            .ok_or_else(|| anyhow!("Hero record {} not found", hero_record_id))?;
        let members = record["members"].as_array_mut()
            .ok_or_else(|| anyhow!("Members array not found in hero record"))?;

        let need_update_max = new_max_soldiers.is_some();
        let need_update_lost = final_soldiers_lost != current_soldiers_lost;
        let mut max_updated = !need_update_max;
        let mut lost_updated = !need_update_lost;

        for member in members.iter_mut() {
            match member["name"].as_str() {
                Some("maxSoldiers") if !max_updated => {
                    member["value"] = Value::Number(final_max.into());
                    max_updated = true;
                }
                Some("soldiersLost") if !lost_updated => {
                    member["value"] = Value::Number(final_soldiers_lost.into());
                    lost_updated = true;
                }
                _ => {}
            }
        ...
        if !max_updated {
            ...
            let members = json_value["records"][hero_record_id]["members"].as_array_mut()
                .ok_or_else(|| anyhow!("Members array not found in hero record"))?;
            members.push(serde_json::json!({"name": "maxSoldiers", "type": "Int32", "value": final_max}));
            info!("Inserted missing maxSoldiers field for hero {} with value {}", hero_record_id, final_max);
        }
        if !lost_updated {
            return Err(anyhow!("soldiersLost field not found for hero {}", hero_record_id));
        }

        let log_msg = log_parts.join("�?);
        info!("Modified squad members for hero {}: {}", hero_record_id, log_msg);
        Ok(log_msg)
    }
    ...
    pub fn get_hero_details(json_value: &Value, hero_record_id: &str) -> Result<HeroDetails> {
        ...
        info!("英雄 {} 的完�?members 结构�?, hero_record_id);
    ...
    pub fn add_grail_to_inventory(json_value: &mut Value) -> Result<()> {
        ...
        let (class_type_id, class_type_name) = Self::find_serializable_hero_upgrade_class_type(json_value)
            .ok_or_else(|| anyhow!("找不�?SerializableHeroUpgrade 类型信息"))?;
        ...
        if null_multiple_count <= 255 {
            warn!(
                "�?背包圣杯已写�?JSON，但存储时可能失败：\
                BinaryArray 槽位原来�?NullMultiple(count={})�? 字节），\
                替换�?Reference�? 字节）会导致记录字节长度变化。\
    ...
    pub fn remove_grail_from_inventory(json_value: &mut Value) -> Result<()> {
        ...
        let last_item_json_index: usize;
        {
            ...
            last_item_json_index = found.ok_or_else(|| anyhow!("找不到背包最后一个有效槽�?))?;
        }
    ...
    pub fn remove_item_from_inventory(json_value: &mut Value, upgrade_code: &str) -> Result<()> {
        ...
        let item_json_index: usize;
        {
            ...
            item_json_index =
                found.ok_or_else(|| anyhow!("背包中没�?{} 可移�?, upgrade_code))?;
        }
        ...
        let last_item_json_index: usize;
        {
            ...
            last_item_json_index =
                found.ok_or_else(|| anyhow!("找不到背包最后一个有效槽�?))?;
        }
    ...
    pub fn set_item_count(
        ...
        if target < 0 {
            return Err(anyhow!("{} 数量不能为负�?, upgrade_code));
        }
